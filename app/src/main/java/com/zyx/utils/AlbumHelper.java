...
public class AlbumHelper {
	...
	void buildImagesBucketList() {
		...
		try {
			...
			if (cur.moveToFirst()) {
				...
				do {
					...
					int count = Parse.getInstance().parseInt(
							bucket.get("count"));
					bucket.put("count", ++count);
					Map<String, Object> imageItem = new HashMap<String, Object>();
		...
		Iterator<Entry<String, Map<String, Object>>> itr = bucketList
				.entrySet().iterator();
		while (itr.hasNext()) {
			Entry<String, Map<String, Object>> entry = itr
					.next();
			Map<String, Object> bucket = entry.getValue();
			for (int i = 0; i < Parse.getInstance()
					.parseList(bucket.get("imageList")).size(); ++i) {
				Map<String, Object> image = Parse.getInstance()
						.parseList(bucket.get("imageList")).get(i);
			}
		}
